...
class TokenService(TokenServiceInterface):
    async def decode_token(self, token: str) -> dict:
        try:
            ...
            raise_exception(status.HTTP_401_UNAUTHORIZED, 'Could not validate credentials')

    async def encode_token(self, email: str) -> str:
        data = {'sub': email, 'exp': datetime.utcnow() + timedelta(minutes=self.exp_time)}
        return jwt.encode(claims=data, key=self.secret_key, algorithm=self.algorithm)
