...
class AccountPermission:
    ...
    async def __get_account(self, token, account_collection, **_filters):
        ...
        if (account := await account_collection.find_one(filter=_filters)) is None:
            raise_exception(status.HTTP_404_NOT_FOUND, 'Account not found')
        return AccountSchema(**account)

    async def get_current_user(self, token: str = Depends(OAUTH_TOKEN),
                               account_collection=Depends(get_account_collection)):
        return await self.__get_account(
            is_active=True, account_type=AccountType.CUSTOMER.value, token=token,
            account_collection=account_collection
        )

    async def get_superuser(self, account_collection=Depends(get_account_collection),
                            token: str = Depends(OAUTH_TOKEN)):
        return await self.__get_account(
            is_active=True, is_superuser=True, account_collection=account_collection,
            account_type=AccountType.SUPERUSER.value, token=token)

    async def get_owner_user(self, token: str = Depends(OAUTH_TOKEN),
                             account_collection=Depends(get_account_collection)):
        return await self.__get_account(
            is_active=True, is_agent=True, account_type=AccountType.AGENT.value, token=token,
            account_collection=account_collection
        )
