...
class ApartmentServices:
    ...
    async def create_apartment(self, account, apartment: CreateApartmentSchema,
                               images: Optional[list[UploadFile]] = None) -> ApartmentSchema:
        return await self.__repository. \
            create_apartment(account=account, apartment=apartment, images=images)
    ...
    async def update_apartment(self, account, apartment_id: str,
                               ...
                               images: Optional[list[UploadFile]] = None) -> ApartmentSchema:
        return await self.__repository. \
            update_apartment(account=account, apartment_id=apartment_id, apartment=apartment, images=images)
    ...
    async def search_apartment(self, search_data: SearchApartmentSchema, skip: int, limit: int):
        return await self.__repository.search(search_data=search_data, skip=skip, limit=limit)

    async def delete_image(self, image_id: str, account):
        return await self.__repository.delete_image(image_id=image_id, account=account)
