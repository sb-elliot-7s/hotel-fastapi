...
class ApartmentRepositories(AggregationMixin, ApartmentRepositoriesInterface):
    ...
    async def __get_apartment(self, apartment_id: str):
        if (apartment := await self.__apartment_collection
                .find_one(self.filter_objects(_id=ObjectId(apartment_id)))) is None:
    ...
    async def __get_hotel(self, hotel_id: str, account_id: str):
        ...
        if not (hotel := await self.__hotel_collection.find_one(filter=_filter)):
            ...
            async with session.start_transaction():
                if await self.__get_hotel(
                    ...
                    await self.__update_count_available_apartments_in_the_hotel(
                        account_id=account.id, hotel_id=apartment.hotel_id, value=1)
                    return apt
    ...
    async def __update_count_available_apartments_in_the_hotel(
        ...
        await self.__hotel_collection.update_one(
            filter=self.filter_objects(_id=ObjectId(hotel_id), account_id=account_id),
    ...
    async def remove_apartment(self, account, apartment_id: str):
        ...
        async with await client.start_session() as session:
            async with session.start_transaction():
                apt_filter = self.filter_objects(_id=ObjectId(apartment_id), account_id=account.id)
    ...
    async def update_apartment(
        ...
        if (apartment := await self.__apartment_collection.find_one_and_update(
                filter=self.filter_objects(_id=ObjectId(apartment_id), account_id=account.id),
                update={
        ...
        async with await client.start_session() as session:
            async with session.start_transaction():
                ...
                apartment = await self.__apartment_collection.find_one(
                    self.filter_objects(account_id=account.id, images=self.check_in([ObjectId(image_id)])))
                await self.__apartment_collection \
                    .update_one(filter=self.filter_objects(_id=apartment['_id']),
